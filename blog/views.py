...
def post_detail(request, year, month, day, post):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            # create comment object but not going to save because need to assign post
            new_comment = comment_form.save(commit=False)
            # assign a post to comment
...
def post_share(request, post_id):
    # now retrieve post with post id
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # form field is passed validation
            cd = form.cleaned_data
            post_url = request.build_absolute_uri(post.get_absolute_url())
            subject = f"{cd['name']} recommends you to rea" \
                      f"d "f"{post.title}"
            message = f"read{post.title} at {post_url}\n\n"f"{cd['name']}\'s comments :{cd['comments']}"
            send_mail(subject, message, 'kishanpatelofficial@gmail.com', [cd['to']])
            sent = True
    else:
        form = EmailPostForm()
